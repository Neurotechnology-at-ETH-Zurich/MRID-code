# Magnetic Resonance Identification Tags analysis and electrode localization

This repository contains code and resources associated with the Nature Communications publication describing Magnetic Resonance Identification (MRID) Tags for ultra-flexible neural electrodes to identify and localize electrode channels in the brain.

The scripts provided here support electrode localization and analysis used in the study.

## Associated Publication

This repository accompanies the following paper:

Title: Magnetic Resonance Identification Tags for Ultra-flexible Electrodes

Journal: Nature Communications

Authors: Eminhan Özil✥, Peter Gombkoto✥, Athina Apostolelli, Tansel Baran Yasar, Angeliki D. Vavladeli, Markus Marks, Manabu Rohr-Fukuma, Wolfger von der Behrens, Mehmet Fatih Yanik

Year: 2026

If you use this repository or its methods, please cite the publication and repository. You can cite all versions by using the DOI 10.5281/zenodo.18917093.

## Purpose

The code in this repository was developed to support the analysis and electrode localization procedures described in the paper.

## Disclaimer

The repository is provided for transparency and reproducibility of the published work. The code may require adaptation for use in other experimental setups.
