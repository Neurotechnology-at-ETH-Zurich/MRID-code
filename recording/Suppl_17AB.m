...
time_Rat6=[0 16.99 7.99 18.01 2.86 3.13 0.04 11.05 12.97 13.86 22.02 6.89 7.17 6.84 10.21 6.96 3.85];
time_Rat10=[0 0.15 4.95 21.18 0.82 17.04 9.95 8.05 9.99 7.87 6.95 20.83 7.33 5.98 20.04];
...
for num_file=1:length(BundleFileName)
    ...
    %% Extract YYMMDD_HHMMSS part
    timestamps = cellfun(@(x) x(4:end), sortedSessions, 'UniformOutput', false);
    % Convert to datetime
    dt = datetime(timestamps,'InputFormat','yyMMdd_HHmmss');
    % Days between consecutive sessions
    dayDiff = days(diff(dt));
    % Optional: cumulative days from first session
    cumDays = days(dt - dt(1));
    ...
    for i=1:length(sortedSessions)
        ...
        if sum(Active(contains(hemi,'Left')))~=0 && sum(Active(contains(hemi,'Right')))~=0
            ...
            DaysDiff_x_axis(session_counter)=cumDays(i);
    ...
    plot1=plot(subp1,  DaysDiff_x_axis,([Active_per_session.right]./FunctionalChannels_Ref_right).*100,'LineWidth',2,'Color',colors(color_code(num_file,2),:))
    hold on
    plot2=plot(subp1,  DaysDiff_x_axis ,([Active_per_session.left]./FunctionalChannels_Ref_left).*100,'LineWidth',2,'Color',colors(color_code(num_file,1),:))

    ylim(subp1,[80,104]);
    xlim(subp1,[min((DaysDiff_x_axis)), max(DaysDiff_x_axis)]);
...
grid on
    ...
    legend(gca,'show');
    xticks(round(linspace(0, max(DaysDiff_x_axis), 10)))
    % xticklabels([0 17 25 43 46 49 60 73])
    title('Suppl. 17B')
    ...
    hold on
    plot3=plot(subp2,counter_calid_segment_x_plot,[Neurons_active_per_session.right],'LineWidth',2,'Color',colors(color_code(num_file,2),:))

    hold on
    plot4=plot(subp2,counter_calid_segment_x_plot,[Neurons_active_per_session.left],'LineWidth',2,'Color',colors(color_code(num_file,1),:))
